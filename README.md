...
- **主窗口**（Liquid Glass 毛玻璃面板）：顶部 wordmark + 汇总 + 刷新按钮，下面是状态 chips 和**按 VSCode 窗口分组**的会话列表，hover 卡片会发光、出现跳转箭头。
  ...
  **单会话文件夹**直接渲染成一张卡（点 → 精准跳那个终端）。**多会话文件夹**渲染成「组标题 + 子列表」：标题右侧用彩色徽标聚合各状态数量（🔴N 需确认 / 🔵N 运行中 / 🟢N 完成 / ⚪N 闲置，nonzero only，最紧急排前），一眼看清整个窗口状态；点组标题 → 抬起该文件夹窗口，点某条子行 → 精准跳那个终端。
...
- **点过即变灰（仅「完成」）**：点开过的「完成」行立刻回落成灰色「闲置」，表示你已经看过；直到该会话有新动静才恢复彩色。菜单、主窗口、toast 三处点击都生效。
  - **「需确认」不会因点击而变灰**：点行只是跳到终端，你**还没真的批准/确认**。所以它保持红色，直到你在终端里真正操作、hook 删除状态标记（权威信号）后才自然变色。避免「点了就显示闲置，但其实没确认」的误报。
- **排序**：需确认 → 运行中 → 完成 → 闲置（最该管的永远在最上）。
...
| 🟢 薄荷绿 | 完成 | 跑完了等你输入 | ✅ 去看 |
| ⚪️ 灰 | 闲置 | 「完成」已被你点击跳转、落到终端看过，回落成灰直到有新动静 | 已处理 |
| ⚪️ 冷灰 | 闲置 | 没有活动的 transcript（新会话 / hook 还没触发） | 不用管 |
